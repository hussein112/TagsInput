...
class TagsInput{
    ...
    createTag(){
        this.container.querySelectorAll("li").forEach(li => li.remove());
        this.hidden.value = "";
        this.tags.slice().forEach((tag, loop_index) =>{
            ...
            if(loop_index == this.max - 1){ // Last Element
                this.hidden.value += tag.trim();
            }else{
                this.hidden.value += tag.trim() + this.trigger;
            }
    ...
    removeHidden(tag){
        tag = tag.trim();
        let temp = this.hidden.value;

        if(temp.includes(tag + ',')){
            tag = tag + ',';
        }
        this.hidden.value = temp.replace(tag, '');
    }
